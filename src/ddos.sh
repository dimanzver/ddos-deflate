...
detect_ddos()
{
    ...
    if [ "$count" -gt "$SUPER_DDOS_DETECT_CONNECTIONS" ]; then
        ON_SUPER_DDOS=true
    else
        ON_SUPER_DDOS=false
    fi
    ...
    if $ON_SUPER_DDOS; then
        port_connections_max=$ON_SUPER_DDOS_PORT_CONNECTIONS
        no_of_connections_max=$ON_SUPER_DDOS_NO_OF_CONNECTIONS
    fi
}
...
SUPER_DDOS_DETECT_CONNECTIONS=250
ON_DDOS_NO_OF_CONNECTIONS=20
ON_STRONG_DDOS_NO_OF_CONNECTIONS=10
ON_SUPER_DDOS_NO_OF_CONNECTIONS=3
ENABLE_PORTS=false
...
ON_SUPER_DDOS_PORT_CONNECTIONS="80:3:600 443:3:600 20-21:5:600"
FIREWALL="auto"
EMAIL_TO="root"
BAN_PERIOD=600
